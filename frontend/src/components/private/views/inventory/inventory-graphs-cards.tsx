    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6 w-full">
            {stores.map((store) => {
                ...
                const total = items.reduce((acc, item) => acc + (item.minimum_stock ?? 0), 0);
                ...
                return (
                    <Dialog key={store.name}>
                        <DialogTrigger asChild>
                            <Card
                                className="overflow-hidden border border-gray-200 dark:border-gray-800 shadow-[-10px_-10px_30px_4px_rgba(0,0,0,0.08),_10px_10px_30px_4px_rgba(100,100,100,0.15)]
