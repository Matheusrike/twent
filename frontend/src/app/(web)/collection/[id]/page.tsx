        ...
        <div className="flex flex-col lg:flex-row w-full min-h-screen">
          ...
          <div className="hidden lg:flex lg:w-3/5 xl:w-4/5 h-screen sticky top-0 p-4 flex-col gap-4">
            <div className="relative w-full lg:h-2/3 md:h-1/2 rounded-2xl overflow-hidden">
              <Image
                src={product.images?.[0]?.url || "/images/collection/gradient.png"}
                alt={product.name}
                fill
            ...
            <div className="flex gap-4 w-full lg:h-1/3 md:h-1/2">
              <div className="relative w-1/2 h-full rounded-2xl overflow-hidden">
                <Image
                  src={product.images?.[1]?.url || "/images/collection/gradient.png"}
                  alt={product.name}
                  fill
                  className="object-cover object-center bg-gray-300"
                  priority
              ...
              <div className="relative w-1/2 h-full rounded-2xl overflow-hidden">
                <Image
                  src={product.images?.[2]?.url || "/images/collection/gradient.png"}
                  alt={product.name}
                  fill
                  className="object-cover object-center bg-gray-300"
                  priority
                ...
                <div className="space-y-3 text-sm">
                  {product.sku && (
                    <div className="flex justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
                      ...
                      <span className="font-medium dark:text-white">{product.sku}</span>
                    </div>
                  )}
                  {product.specifications.case_material && (
                    <div className="flex justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
                      <span className="text-gray-600 dark:text-red-500">Material</span>
                      <span className="font-medium dark:text-white">{product.specifications.case_material}</span>
                    </div>
                  )}
                  {product.specifications.movement_type && (
                    <div className="flex justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
                      <span className="text-gray-600 dark:text-red-500">Movimento</span>
                      <span className="font-medium dark:text-white">{product.specifications.movement_type}</span>
                    </div>
                  )}
                  {product.specifications.case_diameter && (
                    <div className="flex justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
                      <span className="text-gray-600 dark:text-red-500">Diâmetro</span>
                      <span className="font-medium dark:text-white">{product.specifications.case_diameter}mm</span>
                    </div>
                  )}
                  {product.specifications.glass && (
                    <div className="flex justify-between border-b border-gray-200 dark:border-gray-700 pb-2">
                      <span className="text-gray-600 dark:text-red-500">Vidro</span>
                      <span className="font-medium dark:text-white">{product.specifications.glass}</span>
                    </div>
                  )}
                </div>
              </div>
