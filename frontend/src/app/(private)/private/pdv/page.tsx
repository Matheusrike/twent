    ...
    const processPayment = async () => {
        ...
        try {
            ...
            for (const item of cart) {
                ...
                const [resSale, resFinancial] = await Promise.all([
                    fetch('/response/api/sale', {
                        method: 'POST',
                    ...
                    fetch('/response/api/financial-transaction', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            type: 'INCOME',
                            ...
                            currency: 'BRL',
                            description: 'Venda de produto: ' + item.name,
                ...
                if (!resSale.ok && !resFinancial.ok) {
                    const errSale = await resSale.json();
                    const errFinancial = await resFinancial.json();
                    throw new Error(errSale.message || errFinancial.message || 'Erro ao registrar venda');
                }
            }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 p-4 bg-background ">
            <Card className="lg:col-span-2">
                <CardContent className="p-4">
                    ...
                    {error && (
                        <div className="bg-red-100 text-red-700 p-3 rounded mb-3 text-center text-sm">{error}</div>
                    )}

                    <ScrollArea className="h-[calc(100vh-180px)]">
                        {loading && <p className="text-center py-16 text-muted-foreground">Carregando estoque...</p>}
            ...
            <Card className="flex flex-col">
                <CardContent className="p-4 flex flex-col h-full">
                    ...
                    <ScrollArea className="flex-1 my-2">
                        {cart.length === 0 ? (
                            <p className="text-center py-10 text-muted-foreground text-sm">Carrinho vazio</p>
                        ) : (
                        ...
                        <CardContent className="p-6">
                            {showSuccess ? (
                                <div className="text-center py-12">
                                    ...
                                    <h2 className="text-2xl font-bold text-green-600">Venda concluída!</h2>
                                    <p className="text-muted-foreground mt-3 text-sm">Redirecionando ao PDV...</p>
                                </div>
                            ) : isProcessing ? (
                                <div className="text-center py-12">
                                ...
                                <>
                                    <h3 className="text-2xl font-bold text-center mb-6">Forma de pagamento</h3>
